...
glm::mat4 Transform::GetLocalToAncestorMatrix(SceneNode* ancestor) const {
  ...
  // Column-vector convention: the ancestor-side transform is applied last, so
  // it must be the left operand. Reversing these is only invisible while every
  // matrix in the chain is a pure translation (translations commute).
...
glm::mat4 Transform::GetLocalToWorldMatrix() const {
  ...
  return this->GetLocalToAncestorMatrix(nullptr);
}
