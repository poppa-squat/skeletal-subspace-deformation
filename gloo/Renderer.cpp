...
void Renderer::RecursiveRetrieve(const SceneNode& node, RenderingInfo& info, const glm::mat4& model_matrix) const {
  ...
  glm::mat4 current_matrix = model_matrix * node.GetTransform().GetLocalToParentMatrix();
  if (auto* rc = node.GetComponentPtr<RenderingComponent>()) {
    info.push_back({rc, current_matrix});
  }

  for (size_t i = 0; i < node.GetChildrenCount(); ++i){
    RecursiveRetrieve(node.GetChild(i), info, current_matrix);
  }
}
